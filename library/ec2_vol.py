...
options:
  ...
  state:
    ...
    required: false
    default: present
    choices: ['absent', 'present', 'list', 'import']
    version_added: "1.6"
author: Lester Wade
...
def get_volume(module, ec2, _id=None):
...
def import_volume(module, ec2, zone):

    name = module.params.get('name')
    volume_size = module.params.get('volume_size')
    image_size = module.params.get('image_size')
    image_type = module.params.get('image_type')
    manifest_url = module.params.get('manifest_url')
    zone = module.params.get('zone')

    if not hasattr(ec2, 'import_volume'):
        module.fail_json(msg="Your Boto library is too old and is missing import_volume call")
    
    try:
        import_task = ec2.import_volume(volume_size, zone, description=name, image_format=image_type,
                                        image_size=image_size, manifest_url=manifest_url)
        if not import_task:
            module.fail_json(msg="Failed to start volume import")

        while import_task.state != 'completed':
            time.sleep(3)
            import_task = ec2.get_all_conversion_tasks([import_task.id])[0]
            
        volume = None
        if import_task.volume_id:
            volume = get_volume_by_id(module, ec2, import_task.volume_id)
...
def main():
    ...
    argument_spec.update(dict(
            ...
            device_name = dict(),
            image_size = dict(),
            image_type = dict(default='VMDK'),
            manifest_url = dict(),
            zone = dict(aliases=['availability_zone', 'aws_zone', 'ec2_zone']),
            snapshot = dict(),
            state = dict(choices=['absent', 'present', 'list', 'import'], default='present')
        )
    )
    ...
    if state != 'import' and volume_size and (id or name):
    ...
    if state == 'import':
        volume = import_volume(module, ec2, zone)
        module.exit_json(volume_id=volume.id, device=device_name, volume_type=volume.type, changed=True)
